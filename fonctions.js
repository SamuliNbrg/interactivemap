...
function getDescr(i) {
	...
	text = text.replace("<%>", "<p class='lock'><i class='fa-solid fa-lock' title='Spoiler' onclick='$(\".spoiler\").css(\"visibility\",\"visible\")'></i></p><p class='spoiler'>");
			...
			if (into)
			{
				...
				if (x != 0)
				{
					...
					dot.click(function(){
						...
						if (viewText==1)
						{
							$('#txt').html("<h2>"+this.node.nameP+" <i class='fa-solid fa-thumbtack' title='URL' onclick='showurl(nameP)'></i><i id='eye' class='fa-solid fa-eye-slash' title='"+LgHideDescr+"' onclick='fixText()'></i></h2>"+this.node.nameS+this.node.text);
							$('#txt h3').show();
							$('#txt p').show();
							console.log("setPoints1 "+this.node.nameP);
						}
						else
						{
							$('#txt').html("<h2>"+this.node.nameP+" <i class='fa-solid fa-thumbtack' title='URL' onclick='showurl(nameP)'></i><i id='eye' class='fa-solid fa-eye' title='"+LgShowDescr+"' onclick='fixText()'></i></h2>"+this.node.nameS+this.node.text);
							$('#txt h3').hide();
							$('#txt p').hide();
							console.log("setPoints0 "+this.node.nameP);
						}
						showDescr();
					});
				}
...
function showurl(pinurl) {
	if (viewURL == 0)
	{
		$("#txt h2").after("<h4>https://www.aidedd.org/atlas/index.php?map="+carte+"&l="+lang+"&p="+pinurl+"</h4>");
		viewURL = 1;
	}
	else
	{
...
//	dot.remove();
	...
	for (var i=0; i<groupe.length; i++)
	{
		...
		if (search == nameP || search == nameS)
		{
			...
			if (viewText==1)
			{
				var descr = "<h2>"+nameP+" <i class='fa-solid fa-thumbtack' title='URL' onclick='showurl(nameP)'></i><i id='eye' class='fa-solid fa-eye-slash' title='"+LgHideDescr+"' onclick='fixText()'></i></h2>";
			...
			{
				var descr = "<h2>"+nameP+" <i class='fa-solid fa-thumbtack' title='URL' onclick='showurl(nameP)'></i><i id='eye' class='fa-solid fa-eye' title='"+LgShowDescr+"' onclick='fixText()'></i></h2>";				
				if (nameP != nameS) descr += "<h3>[ "+nameS+" ]</h3>";
				descr += text;
				if (typeof groupe[i].img !== 'undefined') descr += "<p>"+groupe[i].img+"</p>";
				$('#txt').html(descr);
